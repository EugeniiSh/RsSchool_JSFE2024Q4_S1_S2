...
export class PlayField extends Component {
  ...
  public async renderGameFieldContent(
    ...
    const contentData = await this.externalStorage.getData(
      contentInfo.playerProgress.last.level,
    );
    const words = contentData.rounds[0].words[0].textExample;
    let wordsElementArr = words.split(' ').map((word) => {
    ...
    resultLine.appendChildren(
      this.wordContainer.getWordContainerArr(wordsElementArr.length),
    );
