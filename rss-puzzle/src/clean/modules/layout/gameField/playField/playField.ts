...
import {
  IPuzzleWordsData,
  IPuzzleLevelData,
  PuzzleGameExternalStorage,
  TNumberOfLevel,
} from '../../../storage/external';
import {
  PuzzleGameStorage,
...
export class PlayField extends Component {
  ...
  static getShuffleElementArr(elementArr: WordContainer[]): {
  ...
  static calculateAndSetBlockWidth(
    resultLine: ResultLine | InitialContainer,
    wordContainer: WordContainer[],
  ) {
    ...
    wordContainer.forEach((elem) => {
      ...
      wordElemChild.style.width =
        'calc(var(--size-width-result) * var(--size-width-ratio))';

      // === add a variable for adaptability ===
      const wordElemWidthRatio = elemWidth / resultWidth;
      wordElemChild.style.setProperty(
        '--size-width-ratio',
  ...
  protected renderCurrentSentence(
    ...
    this.currentLine.result.appendChildren(
      this.wordContainer.getWordContainerArr(shuffleSentence.length),
    );
    this.currentResultContainer.append(this.currentLine.result);

    PlayField.calculateAndSetBlockWidth(
  ...
  public goToNextSentence(): void {
    ...
    if (oldLevel !== currentLevel || oldRound !== currentRound) {
    ...
    if (!this.contentData)
      throw new Error('No information about current round.');
    if (
      !Number.isInteger(nextSentenceNum) ||
      (nextSentenceNum < 0 && nextSentenceNum > 9)
    )
  ...
  public showRoundResults(): void {
    ...
    const sentenceProgress =
      playerProgress.game.progress[lastGame.level].roundProgress[lastGame.round]
        .completeSentence;

    const currentRoundSentences = this.contentData.rounds[lastGame.round];

    const roundResultsContainer = this.roundResults.getPlayerResultsComponent(
      sentenceProgress,
  ...
  public mutableUpdateUserGameProgress(
    ...
    switch (action) {
      case 'next-sentence': {
        ...
        const isSentenceComplete = round.completeSentence.find(
          (completeSentence) =>
            completeSentence.number === oldLastGame.sentense.number,
        );
        ...
        if (round.isComplete) {
          ...
          if (
            this.contentData.roundsCount > level.completeRoundCount &&
        ...
        if (level.isComplete) {
          const nextLevel = Object.keys(levelProgressList).find((levelNum) => {
            const isNotComplete =
              !levelProgressList[Number(levelNum) as TNumberOfLevel].isComplete;
          ...
          Object.keys(nextRoundProgress).find((roundNum, index) => {
            if (!nextRoundProgress[index]) {
              nextRoundProgress[index] =
                this.localStorage.getStartRoundProgress();
              nextRound = index;
              return true;
            }
          ...
          const lastSentence =
            nextRoundProgress[nextRound].completeSentence.at(-1);
          const nextSentence =
            lastSentence?.number === 0 || lastSentence
        ...
        if (round.isComplete) {
          ...
          Array.from({ length: roundCount }, (_, index) => index).find(
            (roundNum) => {
              if (!currentRoundProgress[roundNum]) {
                currentRoundProgress[roundNum] =
                  this.localStorage.getStartRoundProgress();
                nextRound = roundNum;
                return true;
              }
          ...
          const lastSentence =
            currentRoundProgress[nextRound].completeSentence.at(-1);
          const nextSentence =
            lastSentence?.number === 0 || lastSentence
              ? { number: lastSentence.number + 1, isWithHelp: false }
              : { number: 0, isWithHelp: false };
        ...
        oldLastGame.sentense = {
          number: oldLastGame.sentense.number + 1,
          isWithHelp: false,
        };
        break;
      }
      ...
      case 'custom-choice': {
        ...
        if (!nextLevel) {
          levelProgressList[newLastGame.level] =
            this.localStorage.getStartLevelProgress();
          nextLevel = levelProgressList[newLastGame.level];
        }

        let nextRound = nextLevel.roundProgress[newLastGame.round];
        if (!nextRound) {
          nextLevel.roundProgress[newLastGame.round] =
            this.localStorage.getStartRoundProgress();
  ...
  protected handlerClickWordBlock = (event: Event) => {
    ...
    if (parent.closest(`.${this.style.initialGuess}`)) {
      ...
      const wordBlockComponent = this.currentLine.initial
        .getChildren()
        [position].getChildren()[0];

      this.currentLine.initial.getChildren()[position].cleanInnerHTML();
      ...
      this.currentLine.initial
        .getChildren()
        [initialNewPosition].append(wordBlockComponent);
    }
  ...
  protected revealImageAndInformation(): void {
    ...
    const { year, author, name } =
      this.contentData.rounds[currentRound].levelData;
    const pictureBlock = new Component({
      tag: 'div',
      className: [this.style.completePictureBlock],
      text: '',
    });
    ...
    setTimeout(() => {
      ...
      this.currentResultContainer.toggleClass(
        this.style.hiddenGuessBlock,
        true,
      );
    }, 300);
  }
  ...
  protected isAllSentenceInResultConteiner(): boolean {
    ...
    const currentSentenceCount =
      this.currentResultContainer.getChildren().length;
  ...
  public getPlayFieldInterface(progressPlayer: IStorageGameProgress): {
    playField: PlayField;
    renderInfo: IRenderContentInfo;
  } {
