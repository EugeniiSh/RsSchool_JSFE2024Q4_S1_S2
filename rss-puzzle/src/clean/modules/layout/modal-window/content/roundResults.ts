...
export class RoundResults extends Component {
  ...
  protected getRoundResultsSentences(
    ...
    const iKnowBlock = new Component({
      tag: 'div',
      className: [this.style.knowledgeBlock],
      text: '',
    });
    ...
    const iDontKnowBlock = new Component({
      tag: 'div',
      className: [this.style.knowledgeBlock],
      text: '',
    });
    ...
    roundData.words.forEach((roundSentence, index) => {
      ...
      const isWithHelp = sentenceProgress[index]
        ? sentenceProgress[index].isWithHelp
        : false;
      ...
      const audioComponent = new Component({
        tag: 'div',
        className: [this.style.audioHint],
        text: '',
      });
      audioComponent.setAttribute(
        'data-audio',
        this.externalStorage.getAudioPath(roundSentence),
      );

      const sentenceBlock = new Component({
        tag: 'div',
        className: [this.style.sentenceBlock],
        text: '',
      });
  ...
  public getPlayerResultsComponent(
    ...
    const resultsContainer = new Component({
      tag: 'div',
      className: [this.style.resultsContainer],
      text: '',
    });
