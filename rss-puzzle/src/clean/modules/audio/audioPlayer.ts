...
export class AudioPlayer extends Component {
  ...
  protected addHTML(): IAudioPlayerComponents {
    ...
    const volumeContainer = new Component({
      tag: 'div',
      className: [this.style.volumeContainer],
      text: '',
    });
    const volumeButton = new Component({
      tag: 'div',
      className: [this.style.volumeButton],
      text: '',
    });
    const volumeLine = new Component({
      tag: 'div',
      className: [this.style.volumeLine],
      text: '',
    });
    const volumeLinePersent = new Component({
      tag: 'div',
      className: [this.style.volumeLinePersent],
      text: '',
    });
    ...
    const progressContainer = new Component({
      tag: 'div',
      className: [this.style.progressContainer],
      text: '',
    });
    const progressLine = new Component({
      tag: 'div',
      className: [this.style.progressLine],
      text: '',
    });
    const progressPersent = new Component({
      tag: 'div',
      className: [this.style.progressPersent],
      text: '',
    });
    const totalTime = new Component({
      tag: 'div',
      className: [this.style.totalTime],
      text: '',
    });
    const currentTime = new Component({
      tag: 'div',
      className: [this.style.currentTime],
      text: '',
    });
    ...
    const buttonsContainer = new Component({
      tag: 'div',
      className: [this.style.buttonsContainer],
      text: '',
    });
    const buttonPrev = new Component({
      tag: 'div',
      className: [this.style.btn, this.style.buttonPrev],
      text: '',
    });
    const buttonPlay = new Component({
      tag: 'div',
      className: [this.style.btn, this.style.buttonPlay],
      text: '',
    });
    const buttonNext = new Component({
      tag: 'div',
      className: [this.style.btn, this.style.buttonNext],
      text: '',
    });
  ...
  protected updateProgress(): void {
    ...
    this.dom.progressPersent.getNode().style[this.progressVector] =
      `${progressPercent}%`;
  ...
  protected setProgress(event: Event): void {
    ...
    const clientVector =
      `client${this.progressVector[0].toUpperCase() + this.progressVector.slice(1)}` as
        | 'clientWidth'
        | 'clientHeight';
    const progressLineLength = this.dom.progressLine.getNode()[clientVector];

    const offsetVector =
      `offset${this.progressVector === 'width' ? 'X' : 'Y'}` as
        | 'offsetX'
        | 'offsetY';
    ...
    if (this.progressVector === 'width') {
      ...
      const newProgress =
        ((progressLineLength - currentClickPoint) / progressLineLength) *
        duration;
      this.dom.audioNode.currentTime = newProgress;
      this.dom.progressPersent.getNode().style[this.progressVector] =
        `${newProgress * 100}%`;
  ...
  protected changeVolume(event: Event): void {
    ...
    const clientVector =
      `client${this.volumeVector[0].toUpperCase() + this.volumeVector.slice(1)}` as
        | 'clientWidth'
        | 'clientHeight';
    const volumelineLength = this.dom.volumeLine.getNode()[clientVector];

    const offsetVector =
      `offset${this.volumeVector === 'width' ? 'X' : 'Y'}` as
        | 'offsetX'
        | 'offsetY';
    const currentClickPoint = event[offsetVector];
    ...
    if (this.volumeVector === 'width') {
      ...
      this.dom.volumeLinePersent.getNode().style[this.volumeVector] =
        `${newVolume * 100}%`;
    } else {
      const newVolume =
        (volumelineLength - currentClickPoint) / volumelineLength;
      this.dom.audioNode.volume = newVolume;
      this.dom.volumeLinePersent.getNode().style[this.volumeVector] =
        `${newVolume * 100}%`;
    }
  ...
  protected addEvents(): void {
    if (this.progress) {
      ...
      this.dom.audioTag.addListener('loadeddata', () => {
        this.dom.totalTime.setTextContent(
          this.getTimeCodeFromNum(this.dom.audioNode.duration),
        );
        this.dom.currentTime.setTextContent(
          this.getTimeCodeFromNum(this.dom.audioNode.currentTime),
        );
        this.updateProgress();
      });
      ...
      this.dom.progressLine.addListener('click', (event) =>
    ...
    if (this.volume) {
      ...
      this.dom.volumeLine.addListener('click', (event) =>
