...
export type TCustomEventUINames = Extract<TCustomEventNames, 'anableUI' | 'disableUI'>
export type TSubNameEventUI = 'all'| 'no-one' | 'only-me' | 'except-me';

type TAll = { type: Extract<TSubNameEventUI, 'all'| 'no-one'>, sours: null };
type TOther = { type: Exclude<TSubNameEventUI, 'all'| 'no-one'>, sours: Component };
export type TDetailOption = TAll | TOther;

export type TSubEventUIList = Record<TSubNameEventUI, CustomEvent<TDetailOption>>
